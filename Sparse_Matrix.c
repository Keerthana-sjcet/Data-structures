 /*PROGRAM: Sparse Matrix Representation, Addition, Transpose

    DESCRIPTION:
        This program reads two matrices of size (rows × cols),
        converts them into sparse matrix (3–tuple) format,
        displays both sparse matrices,
        computes the sum of the two sparse matrices,
        and finally generates and displays the transpose
        of the sum sparse matrix.

    FEATURES:
        ✔ Convert normal matrix to sparse matrix format
        ✔ Display sparse matrices (3-column representation)
        ✔ Add two sparse matrices using merge technique
        ✔ Produce sparse representation of sum matrix
        ✔ Compute transpose of the sum sparse matrix

    FORMAT OF SPARSE MATRIX (3–TUPLE):
